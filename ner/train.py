...
def main():
    ...
    parser.add_argument('--en_path', default='raw_data/MSRA_train_en.txt', type=str, required=False, help='原始训练语料')
    ...
    parser.add_argument('--log_step', default=1000, type=int, required=False, help='多少步汇报一次loss，设置为gradient accumulation的整数倍')
    ...
    unsupervised_path = ['raw_data/MSRA_test.txt', 'raw_data/renmin_train.txt']
    unsupervised_en_path = ['raw_data/MSRA_test_en.txt', 'raw_data/renmin_train_en.txt']
    ...
    if raw:
        ...
        build_files(data_path=raw_data_path, tokenized_data_path=tokenized_data_path, 
                    en_path=en_path, unsupervised_path=unsupervised_path,
                    unsupervised_en_path=unsupervised_en_path, num_pieces=num_pieces,
                    full_tokenizer=full_tokenizer)
        print('files built')

    model = Network.from_pretrained('bert-base-multilingual-cased', num_labels=17)
    vocab_size = model.config.vocab_size
    # model.load_state_dict(torch.load('model/model_epoch59/model.pt'))
    model.train()
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        for i in x:
            ...
            for step, (batch_inputs, batch_segments, batch_tags, attention_mask, batch_lm_lbls) in enumerate(make_batches(samples, u_samples, batch_size, full_tokenizer)):
                sum_loss_tag, sum_loss_lm = model.forward(batch_inputs, token_type_ids=batch_segments,
                        labels=batch_tags, masked_lm_labels=batch_lm_lbls, attention_mask=attention_mask)
                loss_tag = sum_loss_tag.sum()/(batch_tags!=-100).sum()
                loss_lm = sum_loss_lm.sum()/(batch_lm_lbls!=-100).sum()
        ...
        model.eval()
        #result_train = evaluate('raw_data/MSRA_train.txt', model, full_tokenizer, batch_size)
        #print('train results:')
        #print(result_train)
        #result_test = evaluate('raw_data/MSRA_test.txt', 'raw_data/MSRA_test_en.txt', model, full_tokenizer, batch_size)
        #print('test results:')
        #print(result_test)
        result_cd = evaluate('raw_data/renmin_test.txt', 'raw_data/renmin_test_en.txt', model, full_tokenizer, batch_size)
    ...
    torch.save(model_to_save.state_dict(), output_dir + 'final_model/model.pt'.format(epoch+1))
